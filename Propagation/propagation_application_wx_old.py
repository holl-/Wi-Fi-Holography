...
class MatPlotLibCanvas(wx.Panel):
    def __init__(self,parent, dr, pos, size):
    ...
    def plot_external(self, image, savefile, use_tum_jet=True, flip_ud=True, fontsize=8):
        ...
        plt.savefig(savefile+".pdf", transparent=True)
        plt.savefig(savefile+".png", transparent=True)
        fig.show()
...
class WXPropagationFrame(wx.Frame):
    def __init__(self, parent, title, propagators, wf_offset, wf_size, save_dir, prefix, multithreaded, pinhole_rad, max_dist_cm):
        wx.Frame.__init__(self,parent,title=title,size=(650,650), style=wx.MINIMIZE_BOX|wx.SYSTEM_MENU|
        ...
        self.sp.SplitHorizontally(self.canvas, self.p2, 470)
        ...
        self.hibut = wx.Button(self.p2, -1, "Save print", size=(70, 20), pos=(200+70+70, 6))
        ...
        self.locsrc = wx.Button(self.p2, -1, "Locate Source", size=(90, 20), pos=(260, 90))
    ...
    def locate_source(self, event):
        ...
        for y in xrange(- rad_px, rad_px):
            for x in xrange(- rad_px, rad_px):
                if x**2 + y**2 <= rad_px**2:
                    ...
                    print "Adding ",abs(self.wavefront[max_y+y, max_x+x])**2,"from index",x,y
        ...
        print "----- Fit -------"
        cut_threshold = 0.25 * x_values[max_x]
        ...
        fit_sigma_error = pcov[2,2]
        print "popt",popt
        print "pcov",pcov
        print "Sigma:",fit_sigma,"+-",fit_sigma_error

        gaus_fit_data = [gaus_f(x, *popt) for x in x_axis]



        #x_m, y_m = self.canvas.pixel_to_meters(x, y, self.wavefront.shape)
        x_m = x_axis[max_x]
        y_m = y_axis[max_y]
        ...
        pylab.plot(x_axis, gaus_fit_data, "b+", label="Gaus fit x")
    ...
    def show_direct(self, display_z= 101, frequency=-1):
        self.darkfield_propagators = None
        self.frequency = frequency
        self.freq_slider.SetValue(frequency)
...
def create_propagator(frequency_index, propagators, wavefront, dr):
...
def launch(wavefront_file, dr=0.01, max_dist=15, save_dir="../", multithreaded=True, pinhole_rad=0.0002, display_z=101, initial_frequency_index=-1, enable_darkfield=True, source_z=223, save_diagram=False):
